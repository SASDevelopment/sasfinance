<?php
//include($_SERVER['DOCUMENT_ROOT'].'/sasfinance/assets/functions.royalties3.php');

$timestamp = date("Y-m-d");
$lastdaymonth = date("Y-m-t", strtotime($a_date));
if($timestamp == $lastdaymonth) {
    mail('pclark@salemauthorservices.com', date('Y-m-d H:i:s') . ' royalties3_load_currency Last Day.', 'royalties3_load_currency.php', "From: authorsupport@salemauthorservices.com\r\nReply-to: authorsupport@salemauthorservices.com\r\nContent-type: text/html; charset=iso-8859-1\r\n\n");
    royalties3_load_royalty_currency_data();
}
else
{
    mail('pclark@salemauthorservices.com', date('Y-m-d H:i:s') . ' royalties3_load_currency.', 'royalties3_load_currency.php', "From: authorsupport@salemauthorservices.com\r\nReply-to: authorsupport@salemauthorservices.com\r\nContent-type: text/html; charset=iso-8859-1\r\n\n");
}

//royalties3_load_royalty_currency_data();

function royalties3_currencyConverter($from,$to,$currency_input){
    $ch = curl_init();
    ...
    $data = explode(" ", $data[1]);
    $amnt = round($data[0], 8);
    $amnt = $amnt * $currency_input;

    $total = number_format($amnt, 4, '.', '');

    if($total == '403' || $total == 0.00) { $total = 'NULL'; }
...
function royalties3_load_royalty_currency_data() {
	...
	foreach ($arr as &$currency_from) {
        ...
        $currency_from_abbr = substr($currency_from, 0, 2);
        $currency_to_abbr = substr($currency_to, 0, 2);

		$query_insert_royalty_currency_data = "insert ignore into sasroyalties.royalty_currency_data (currency_from, currency_to, currency_output, currency_from_abbr, currency_to_abbr, record_timestamp, last_updated_timestamp, record_date) values ('$currency_from', '$currency_to', ".royalties3_currencyConverter($currency_from,$currency_to,$currency_input).", '$currency_from_abbr', '$currency_to_abbr', NOW(), NOW(), CURDATE())";
		$resultinsert_royalty_currency_data=$db->query($query_insert_royalty_currency_data);

		$query_insert_royalty_currency_data = "insert ignore into sasroyalties.royalty_currency_data (currency_from, currency_to, currency_output, currency_from_abbr, currency_to_abbr, record_timestamp, last_updated_timestamp, record_date) values ('$currency_to', '$currency_from', ".royalties3_currencyConverter($currency_to,$currency_from,$currency_input).", '$currency_to_abbr', '$currency_from_abbr', NOW(), NOW(), CURDATE())";
		$resultinsert_royalty_currency_data=$db->query($query_insert_royalty_currency_data);
	}
	$db->close();
}
